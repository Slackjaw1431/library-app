...
export const Loans = () => {
    ...
    useEffect(() => {
        const fetchUserCurrentLoans = async () => {
            if (authState && authState.isAuthenticated) {
                const url = `${process.env.REACT_APP_URL}/books/secure/currentloans`;
    ...
    async function returnBook(bookId: number) {
        const url = `${process.env.REACT_APP_URL}/books/secure/return/?bookId=${bookId}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
    ...
    async function renewLoan(bookId: number) {
        const url = `${process.env.REACT_APP_URL}/books/secure/renew/loan/?bookId=${bookId}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
                        ...
                        <div key={shelfCurrentLoan.book.id}>
                            ...
                            <div className='card d-flex mt-5 mb-3'>
                                <div className='card-body container'>
                                    ...
                                    <hr/>
                                    <p className='mt-3'>
                                        Help other by leaving a review.
                                    </p>
                                    <Link className='btn btn-primary' to={`/checkout/${shelfCurrentLoan.book.id}`}>
                                        Leave a review
                                    </Link>
