...
export const PaymentPage = () => {
    ...
    useEffect(() => {
        const fetchFees = async () => {
            if (authState && authState.isAuthenticated){
                const url = `${process.env.REACT_APP_URL}/payments/search/findByUserEmail?userEmail=${authState.accessToken?.claims.sub}`;
    ...
    async function checkout() {
        ...
        const url = `${process.env.REACT_APP_URL}/payment/secure/payment-intent`;
        const requestOptions = {
            method: 'POST',
            headers: {
        ...
        if (!stripeResponse.ok) {
            ...
            throw new Error('Payment Checkout Error!');
        }
        ...
        stripe.confirmCardPayment(
            ...
            if (result.error) {
                ...
                if (!stripeResponse.ok) {
                    setHttpError(true)
                    setSubmitDisabled(false)
                    throw new Error('Stripe response error!')
                }
                setFees(0);
                setSubmitDisabled(false);
            }
