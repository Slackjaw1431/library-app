...
export const ChangeQuantityOfBook: React.FC<{ book: BookModel, deleteBook: any }> = (props, key) => {
    ...
    async function increaseQuantity() {
        const url = `${process.env.REACT_APP_URL}/admin/secure/increase/book/quantity/?bookId=${props.book?.id}`;
        ...
        if (!quantityUpdateResponse.ok) {
            throw new Error('Quantity increase wrong!');
        }
    ...
    async function decreaseQuantity() {
        const url = `${process.env.REACT_APP_URL}/admin/secure/decrease/book/quantity/?bookId=${props.book?.id}`;
        const requestOptions = {
            method: 'PUT',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
        ...
        const quantityUpdateResponse = await fetch(url, requestOptions);
        if (!quantityUpdateResponse.ok) {
            throw new Error('Quantity decrease error!');
        }
        setQuantity(quantity - 1);
        setRemaining(remaining - 1);
    }
    ...
    async function deleteBook() {
        const url = `${process.env.REACT_APP_URL}/admin/secure/delete/book/?bookId=${props.book?.id}`;
        const requestOptions = {
            method: 'DELETE',
            headers: {
                Authorization: `Bearer ${authState?.accessToken?.accessToken}`,
